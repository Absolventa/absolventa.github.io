...
Die Quellcode als solchen darzustellen müssen die Zeilen nur mittels Tab (oder 4 Leerzeichen) eingerückt werden. Wenn jedoch Syntax-Highlighting unterstützt werden soll, muss der Code mit <code>{</code><code>% highlight ruby %}</code> eingeleitet werden. Die letzte Zeile schließt ihn mit <code>{</code><code>% endhighlight %}</code> ab. 

Markdown unterstützt hierbei im Übrigen alle gängigen Sprachen und stellt den Quellcode spezifisch dar. Dazu muss nur die entsprechende Sprache am Anfang angegeben werden, z.B. für Java <code>'{</code><code>% highlight java %}</code> ... <code>{</code><code>% endhighlight %}</code>.
